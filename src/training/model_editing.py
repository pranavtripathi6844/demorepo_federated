...
class IterativeMaskGenerator:
    ...
    def generate_mask(self, model: torch.nn.Module, 
        ...
        local_model.freeze(0)  # Unfreeze all parameters
        
        # Disable gradients for non-backbone layers
    ...
    def _configure_parameter_gradients(self, model: torch.nn.Module):
        ...
        for name, param in model.named_parameters():
            if any(excluded in name for excluded in ['embed', 'cls_token', 'backbone.norm', 'head']):
